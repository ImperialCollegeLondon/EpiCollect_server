...
class CreateProject(webapp.RequestHandler):
  def post(self):
    ...
    if projectName.find(" ")>=0 or projectName.find("\t")>=0 or projectName.find("\n")>=0 or projectName.find(",")>=0 or projectName.find("&")>=0:
      self.response.out.write("Error: Spaces, tabs, commas and/or ampersands are not presently allowed in project names.")
      return
...
class EditProject(webapp.RequestHandler):
  def post(self):
    ...
    if project is None:
      ...
      self.response.out.write(str(templateValues))
      return
...
class DoCreateOrEditForm(webapp.RequestHandler):
  def post(self):
    ...
    #  return
    if users.get_current_user() is None:
      self.response.out.write("Error: You are not logged in <a href=\""+users.create_login_url('/project.html?name=' + projectName) +"\">Login</a>")
      return
...
class MiniDomTest(webapp.RequestHandler):
  def get(self):
    ...
    form = project.getLatestForm()
    formHandler = ec.FormHandler()
    self.response.out.write(str(formHandler.getFullFields(form)))
